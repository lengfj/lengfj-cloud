...
public class XxlJobServiceImpl implements XxlJobService {
	...
	public ReturnT<String> add(XxlJobInfo jobInfo) {
		//重复校验
		if(StringUtils.isNotEmpty(jobInfo.getExecutorHandler())){
			int jobIsExist = xxlJobInfoDao.jobIsExist(jobInfo.getJobGroup(),jobInfo.getExecutorHandler());
			if(jobIsExist>0){
				logger.info("executorHandler已存在", JSONUtils.toJSONString(jobInfo));
			}
		}
		// valid base
